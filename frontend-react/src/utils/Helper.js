...
export function useSliceSelector(pageName, keys) {
  return useMapSelector(globalState => {
      return keys.map(key => globalState[pageName][key]);
    }
  );
}
...
class SliceStore {
  ...
  dispatch(action) {
    this._store.dispatch({type:this._namespace + '/' + action.type, payload: action.payload});
  }
  ...
  setState(payload) {
    this._store.dispatch({type: this._namespace + '/setState', payload: payload});
  }
